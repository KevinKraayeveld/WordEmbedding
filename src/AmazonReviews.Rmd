...
if(get_new_data){
  ...
  if(small_data){
    file_path_train <- paste0("../data/", preprocessing_method, "_train_small.csv")
    df <- fread(file_path_train)
    file_path_test <- paste0("../data/", preprocessing_method, "_test_small.csv")
    test <- fread(file_path_test)
  } else{
    file_path_train <- paste0("../data/", preprocessing_method, "_train.csv")
    df <- fread(file_path_train)
    file_path_test <- paste0("../data/", preprocessing_method, "_test.csv")
    test <- fread(file_path_test)
  }
...
```{r embedding_model, echo = FALSE, warning = FALSE}
