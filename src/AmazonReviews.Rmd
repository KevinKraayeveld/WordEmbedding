...
# - "fastText"
# - "BERT"
embedding_model <- "word2vec_skipgram"
...
if(get_new_data){
  ...
  if(small_data){
    df <- fread("../data/tokenized_reviews_small.csv")
  } else{
...
if(create_new_vectors){
  ...
  source(path)
  if(small_data){
    path2 <- paste0("../data/Vectorized-Reviews/", vectorization_method,"_", embedding_model, "_small.csv")
  }else{
    path2 <- paste0("../data/Vectorized-Reviews/", vectorization_method,"_", embedding_model, ".csv")
  }
  fwrite(df, path2)
  } else{
    if(small_data){
      file_name <- paste0(vectorization_method, "_", embedding_model, "_small.csv")
    }else{
      file_name <- paste0(vectorization_method, "_", embedding_model, ".csv")
    }
