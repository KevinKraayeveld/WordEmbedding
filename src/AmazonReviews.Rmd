...
# - "word2vec_skipgram"
# - "GloVe"
# - "fastText" BROKEN: cannot handle OOV
# - "BERT"
# - "ELMO" BROKEN: tensorflow error
# - "pretrained_word2vec" VECTORIZATION TOO SLOW
# - "pretrained_glove"
# - "pretrained_fasttext"
# - "Lexicon"
...
if(tolower(embedding_model) %in% c("word2vec_cbow", "word2vec_skipgram", "pretrained_word2vec", "pretrained_glove", "glove")){
  no_oov <- T
} else{
  no_oov <- F
}
...
if(get_new_embeddings){
  ...
  if(!tolower(embedding_model) == "lexicon"){
    ...
    lexicon_path <- "../data/Predictions/Lexicon.csv"
    fwrite(test, lexicon_path)
  }
} else{
  if(small_data){
