...
if(get_new_data){
  source("Preprocessing/training_test_split.R")
} else if(!get_new_data & (get_new_predictions | create_new_vectors)){
  if(small_data){
    df <- fread("../data/tokenized_train_small.csv")
    test <- fread("../data/tokenized_test_small.csv")
  } else{
    df <- fread("../data/tokenized_train.csv")
    test <- fread("../data/tokenized_test.csv")
  }
