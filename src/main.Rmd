...
packages <- c("data.table", "parallel", "fastTextR")
...
# - "logistic_regression"
# - "ridge_regression"
prediction_model <- "ridge_regression"
...
if(get_new_data){
  ...
  source(path)
} else if(!get_new_data & create_new_vectors){
  if(small_data){
  ...
  if(tolower(preprocessing_method) == "minimal_cleaning"){
    train$Review_Tokens <- strsplit(train$Review, split = " ", fixed = T)
    test$Review_Tokens <- strsplit(test$Review, split = " ", fixed = T)
  } else{
...
if(get_new_embeddings){
  ...
  if(tolower(embedding_model) == "fasttext"){
